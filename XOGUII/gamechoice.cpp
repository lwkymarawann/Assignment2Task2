...
#include "xo.h"
GameChoice::GameChoice(QString z, QWidget *parent)
    : QDialog(parent), name(z)  // Use QDialog constructor
{
    ...
    QPixmap scaledPixmap1 = pixmap1.scaled(game1->size() *0.8, Qt::KeepAspectRatio, Qt::SmoothTransformation);
    ...
    QPixmap scaledPixmap2 = pixmap2.scaled(game2->size() * 0.8, Qt::KeepAspectRatio, Qt::SmoothTransformation);
    game2->setIcon(scaledPixmap2);
    game2->setIconSize(game2->size());
    game2Label = new QLabel("4X4 TIC-TAC-TOE", game2); // Set parent to game2
    ...
    connect(game1, &QPushButton::clicked, this, &GameChoice::ongame1ButtonClicked);
    connect(game2, &QPushButton::clicked, this, &GameChoice::ongame2ButtonClicked);
...
void GameChoice::ongame1ButtonClicked(){
    this->close();
    ulti = new XO(0, name); // ultimate tic tac toe
    ulti->exec();
}
void GameChoice::ongame2ButtonClicked(){
    this->close();
    sus = new XO(1 , name); // sus tic tac toe
    sus->exec();
}
void GameChoice::onBackButtonClicked() {
    this->close();
    MainWindow *main = new MainWindow();
    main->showFullScreen();    // This will show the parent (MainWindow)
}
